...
class PDOEasy extends PDO
{
     ...
     * @return mixed|null
     */
    public function selectRow(string $table, array $where, string $column = '*')
    {
        // Сбрасываем последние данные
        $this->resetData();
        // Преобразуем условия
        ...
        foreach ($columns_where AS &$el) $el = '"' . $el . '" = ?';
        // Формируем запрос
        $stmt = $this->prepare('SELECT ' . $column . ' FROM ' . $table . ' WHERE ' . implode(' AND ', $columns_where) . ' LIMIT 1');
        // Подстваляем параметры
        $stmt->execute(array_values($where));
        // Устанавливаем значения
        $this->queryRow = $stmt->rowCount();
        $this->isExistQuery = (bool) $this->queryRow;
        $this->queryData = $this->isExistQuery ? $stmt->fetch($this->getAttribute(PDO::ATTR_DEFAULT_FETCH_MODE)) : null;
        unset($stmt);
        return $this->queryData;
    }
     ...
     */
    public function selectAll(string $table, array $where = [], string $column = '*')
    {
        // Сбрасываем последние данные
        $this->resetData();
        // Формируем запрос
     ...
     * @return null|string
     */
    public function selectCount(string $table, array $where = []): ?int
    {
        // Сбрасываем последние данные
        $this->resetData();
        // Формируем запрос
        ...
        $this->queryRow = $stmt->rowCount();
        $this->isExistQuery = (bool) $this->queryRow;
        $this->queryData = (int) $stmt->fetchColumn();
        unset($stmt);
        return $this->queryData;
    }
    ...
    public function update(string $table, array $set, array $where = []): int
    {
        ...
        $this->isExistQuery = $this->queryData;
        $this->queryRow = $stmt->rowCount();
    ...
    public function delete(string $table, array $where): ?int
    {
        ...
        foreach ($columns_where AS &$el) $el = '"' . $el . '" = ?';
        // Формируем запрос
        $stmt = $this->prepare('DELETE FROM ' . $table . ' WHERE ' . implode(' AND ', $columns_where));
        // Подстваляем параметры
        $stmt->execute(array_values($where));
        // Устанавливаем значения
        $this->queryRow = $stmt->rowCount();
